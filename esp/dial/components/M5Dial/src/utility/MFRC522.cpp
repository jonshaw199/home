...
void MFRC522::PCD_WriteRegister(	uint8_t reg,		///< The register to write to. One of the PCD_Register enums.
    ...
    M5.In_I2C.writeRegister(_chipAddress, reg, &value, 1, 100000);
...
void MFRC522::PCD_WriteRegister(	uint8_t reg,		///< The register to write to. One of the PCD_Register enums.
  ...
  M5.In_I2C.writeRegister(_chipAddress, reg, values, count, 100000);
	// Wire.beginTransmission(_chipAddress);
	// Wire.write(reg);
...
uint8_t MFRC522::PCD_ReadRegister(	uint8_t reg	///< The register to read from. One of the PCD_Register enums.
    ...
    M5.In_I2C.readRegister(_chipAddress, reg, &value, 1, 100000);
	// Wire.beginTransmission(_chipAddress);
	// Wire.write(reg);
	// Wire.endTransmission();
...
void MFRC522::PCD_ReadRegister(	uint8_t reg,		///< The register to read from. One of the PCD_Register enums.
    ...
    M5.In_I2C.readRegister(_chipAddress, reg, values, count, 100000);
 ...
 */
const __FlashStringHelper *MFRC522::GetStatusCodeName(uint8_t code	///< One of the StatusCode enums.
										) {
	switch (code) {
 ...
 * @return const __FlashStringHelper *
 */
const __FlashStringHelper *MFRC522::PICC_GetTypeName(uint8_t piccType	///< One of the PICC_Type enums.
										) {
	switch (piccType) {
